...
import { GoogleSearch, QueryEngine } from '../../interface/Search'
...
export class Search extends Workers {
    ...
    public async doSearch(page: Page, data: DashboardData) {
        ...
        // Generate search queries using QueryCore (multi-source: Google Trends, Wikipedia, Reddit, Local)
        ...
        // Use configured query engines or default to all sources
        const sourceOrder: QueryEngine[] = this.bot.config.searchSettings?.queryEngines ?? ['google', 'wikipedia', 'reddit', 'local']

        let rawQueries = await queryCore.queryManager({
            shuffle: true,
            related: true,
            langCode: lang,
            geoLocale: geo,
        ...
        // Convert to GoogleSearch format for compatibility with existing logic
        let googleSearchQueries: GoogleSearch[] = rawQueries.map(q => ({ topic: q, related: [] }))

        // Fallback: if QueryCore failed or insufficient, sample from local queries file
        if (!googleSearchQueries.length || googleSearchQueries.length < 10) {
            this.bot.log(this.bot.isMobile, 'SEARCH-BING', 'Primary QueryCore sources insufficient, falling back to local queries.json', 'warn')
            try {
    ...
    // NOTE: getGoogleTrends and extractJsonFromResponse moved to QueryEngine.ts
