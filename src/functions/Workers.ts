...
const ACTIVITY_DELAYS = {
    THROTTLE_MIN: 500,   // Moderate: Was 200
    THROTTLE_MAX: 1000,  // Moderate: Was 500
    ACTIVITY_SPACING_MIN: 1000, // Moderate: Was 500
    ACTIVITY_SPACING_MAX: 2000  // Moderate: Was 1000
} as const
...
export class Workers {
    ...
    async doPunchCard(page: Page, data: DashboardData) {
        ...
        for (const punchCard of punchCardsUncompleted) {
            ...
            try {
                ...
                this.bot.log(this.bot.isMobile, 'PUNCH-CARD', `Skipped broken Punch Card "${punchCard.parentPromotion.title}": ${msg}`, 'warn')
                continue
            }
    ...
    async doMorePromotions(page: Page, data: DashboardData) {
        ...
        // User-requested blacklist for useless "Search on Bing" tiles (Hourglass/Locked icons)
        const BLACKLIST_TILES = [
            'Know your score',
            'Save more today',
            'Sleep better',
            'Treat your pet',
        ...
        const activitiesUncompleted = morePromotions?.filter(x => {
            ...
            const isBlacklisted = BLACKLIST_TILES.some(bg => {
                const keyword = bg.toLowerCase()
                return title.includes(keyword) || name.includes(keyword)
            })

            if (isBlacklisted && !x.complete) {
                this.bot.log(this.bot.isMobile, 'MORE-PROMOTIONS', `[BLACKLIST] Skipped useless tile: "${x.title || x.name}"`, 'warn')
                return false
            }
