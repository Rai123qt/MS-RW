...
export function loadAccounts(): Account[] {
    try {
        ...
        // Shuffle accounts for anti-detection (User requested)
        for (let i = enabledAccounts.length - 1; i > 0; i--) {
            const j = Math.floor(Math.random() * (i + 1));
            [enabledAccounts[i], enabledAccounts[j]] = [enabledAccounts[j]!, enabledAccounts[i]!];
        }
